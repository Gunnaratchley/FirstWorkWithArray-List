...
// In general the word list means duplicates.
// An array can be a list of items.
// We do not like to use arrays because they have a very
// serious problem.
// The size is static (fixed). This is a problem.
// But arrays do one thing well, random access to elements
// because each element has an index.
// array list gives us random access AND will grow as needed.
// The array list will hide an actual array from the user
// and we will allocate new memory as required.
...
namespace nwacc {
    ...
    public:
        ...
        {
            // Here we create a new array list with a given
			// capacity.
	        // The work, we need to make the array.
        	// We have the size.
    		...
    		data{ nullptr }
        {
        	...
        	// So the only option is to iterate and copy each element.
            this->data = new T[this->my_capacity];
        ...
        ~array_list()
        {
        	// We delete[] because data is an array.
        	// If we only delete data, then we only delete
        	// the first element of the array and the other
        	// data is lost to memory we know not where. 
            delete[] this->data;
        }
        ...
        bool empty() const
        {
        	// Just because the array list is empty
			// DOES NOT mean the array (data) is empty!
            return this->size() == 0;
        }
         ...
         * This capacity is not necessarily equal to the size. 
        ...
        // Just for testing, overriding the << operator so I can display the list and show it works
        friend std::ostream& operator<<(std::ostream& out, const array_list& list) {
            ...
            for (auto index = list.size(); index < list.capacity(); index++) {
                out << "nill" << " ";
            }
    ...
    private:
    	...
    	// The max number of elements I can have in the current
    	// array (data)
    	//
    	...
    	/**
    	 * The backing array which contains the list data. 
    	 */
        T* data;
    };
